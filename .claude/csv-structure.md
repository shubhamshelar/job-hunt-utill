...
| `data/catalog.csv` | `scripts/enrich_jobs.py` | one row per unique `job_url` | first-seen snapshot (empty `description` backfilled from the latest snapshot that has one) + deterministic enrichment |
...
- **LinkedIn descriptions**: jobspy only fetches LinkedIn descriptions when `linkedin_fetch_description=True` (extra GET per job; silently None on login-walls) — `scraper.py` passes it since 2026-08-15. Before that, 100% of LinkedIn rows had empty descriptions (Indeed ~100% coverage via embedded JSON).
