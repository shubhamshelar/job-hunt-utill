...
1. **scraper.py** — reads `config.py`: `TITLES × LOCATIONS × SITES × RESULTS_PER_SEARCH`. **LinkedIn** is scraped directly from the guest jobs API (`jobs-guest/jobs/api/seeMoreJobPostings/search`): one OR-combined keyword search per location (all titles at once, cap = `RESULTS_PER_SEARCH × len(TITLES)`) so pagination reaches the long tail instead of re-fetching overlapping page-1 cards per title; `search_title` is post-tagged by first config-title substring match (else `Other`). LinkedIn job detail pages (description markdown, direct apply URL, emails, company industry/logo) are fetched afterwards with a 6-worker ThreadPoolExecutor, **only for URLs not in `seen_jobs.csv`**. **Indeed/Google** go through `jobspy.scrape_jobs` per title/location pair as before. All rows get `search_title`/`search_location`, concatenated, deduped by `job_url`, rows missing `date_posted` dropped, written to raw CSV. Direct LinkedIn scraping exists because jobspy 1.1.82's card parser broke (LinkedIn renamed the listdate class to `job-search-card__listdate--new` → jobspy returned None dates → every LinkedIn row was silently dropped by the no-date filter); our parser handles the new class plus a relative-date ("19 hours ago") fallback.
...
- `scraper.py` silently removes `glassdoor` (no India support) and `zip_recruiter` (removed entirely — persistent 403s, zero results in history) from `SITES`, hardcodes `country_indeed="India"`. LinkedIn never goes through jobspy: it is scraped from the guest API directly (OR search per location, pagination cap `RESULTS_PER_SEARCH × len(TITLES)`, 2.5-4s sleep between pages, 429 → stop that search) with a 6-worker parallel detail fetch, **globally throttled to ~1 req/s** (the guest API login-walls on sustained bursts — ~2 req/s walls after ~30 requests, ~1 req/s recovers 100%), for unseen URLs **plus seen URLs whose catalog description is still empty**, with one retry pass after a 20s cooldown for login-walled misses (5s timeout each). The raw CSV schema is pinned in `RAW_COLUMNS` and must stay jobspy-compatible (enrich/filter_unseen read it).
